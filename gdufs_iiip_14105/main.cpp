...
////    dba.close();
//    /*get context from txt or dat file*/
//    read_data_form_file("/home/zice/wanglianxi/微博论文分析.TXT",doc_pack);
...
int main()
{
    ...
    string str = prep.segmentation("以国内财经网站和讯微博为例,使用计量学方法,对用户特性进行统计分析,并使用可视化软件Pajek进行可视化分析。研究表明,微博用户的特性,关注者数、被关注者数和博文数均具有统计特性,地域差异明显;另外,两种类型的用户群体之和占用户总体的近90%,具有很强的代表性,为深入研究微博用户行为提供参考。");
    cout<<str<<endl;
    string sep = " ";
    vector<string> splitW;
    cout<<"1"<<endl;
    common::split(splitW,str,sep);
    cout<<"the size:"<<splitW.size()<<endl;
    cout<<"2"<<endl;
    for(vector<string>::size_type i = 0;i<splitW.size();i++)
    {
        cout<<splitW[i]<<endl;
    }
    cout<<"3"<<endl;
